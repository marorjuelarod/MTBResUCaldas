...
def tables(tests,outPrefix):	
	...
	if tests["X"]==1:
		try:
			...
			print(f"file not found: {outPrefix}_XpertClassic.txt")
			sys.exit()	
	...
	if tests["U"]==1:	
		try:
			...
			print(f"file not found: {outPrefix}_XpertUltra.txt")
			sys.exit()	
	...
	if tests["H"]==1:	
		try:
			...
			print(f"file not found: {outPrefix}_HainLPA.txt")
			sys.exit()
	...
	if tests["N"]==1:	
		try:
			...
			print(f"file not found: {outPrefix}_NiproLPA.txt")
			sys.exit()
	...
	if tests["S"]==1:#sanger input is same	format as the Xpert so can use that function
		try:
			...
			print(f"file not found: {outPrefix}_Sanger.txt")
			sys.exit()	
